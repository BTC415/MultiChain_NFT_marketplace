...
function UncontrolledExample({ nftInfo, nftActivity }) {
	return (
				...
				<Tab eventKey="Attributes" title="Attributes">
					{
						nftInfo.attributes ?
							<div className='attributes-list'>
								{
									nftInfo.attributes &&
									nftInfo.attributes.map((item, index) => <Attributes index={index} value={item.value} title={item.trait_type} remarks={item.description} key={index} />)
								}
							</div>
							:
					...
					<div className='activity-list' style={{ width: `100%` }} >
						{
							nftActivity[0] ?
								nftActivity[0].length > 0 ?
									...
									nftActivity[0].map((activity, index) => {
