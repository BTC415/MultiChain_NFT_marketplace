...
const SOLANA_CLUSTER_ENDPOINT = `https://metaplex.devnet.rpcpool.com`;
...
export class SolanaClient {
   ...
   */
  public getAllCollectibles = async (wallets: string[], filters: any[]): Promise<any> => {
    try {
      ...
      const nfts = await Promise.all(
        potentialNFTsByOwnerAddress.map(async ({ mintAddresses }) => {
          ...
          for (let i = 0; i < nonNullInfos.length; i++) {
            ...
            if (filters.length < 1 || filters.find(filter => metadata.updateAuthority === filter.updateAuthority && metadata?.data?.name.indexOf(filter.collectionName) >= 0)) {
              metadataList.push({
                ...metadata,
                ...metadata.data
              });
        ...
        const nfts = await Promise.all(
          potentialNFTsByOwnerAddress.map(async ({ mintAddresses }) => {
            ...
            for (let i = 0; i < nonNullInfos.length; i++) {
              ...
              let metadata: any = await decodeMetadata(nonNullInfos[i].account!.data);
  ...
  private _metaplex = (text: string): Nullable<SolanaNFTMetadata> => {
    ...
    // metaplex standard ggggggggggggggggggggnfts live in arweave, see link below
