...
contract ERC721Sale is ERC721SaleBase {
    ...
    function updateSale(
        ...
        uint256 index = saleTokenIds[contractAddr].findIndex(tokenId);
        require(index < saleTokenIds[contractAddr].length, "Unable to find listed NFT");
        index = saleTokenIdsBySeller[msg.sender][contractAddr].findIndex(tokenId);
        require(index < saleTokenIdsBySeller[msg.sender][contractAddr].length, "Unable to find listed NFT");
    ...
    function updateOffer(
        ...
        uint256 index = tokenIdToSales[contractAddr][tokenId].offerers.findIndex(msg.sender);
        require(index < tokenIdToSales[contractAddr][tokenId].offerers.length, "Unable to find offer");
        uint256 oldPrice = tokenIdToSales[contractAddr][tokenId].offerPrices[index];
        if (price > oldPrice) {
            PaymentLibrary.escrowFund(tokenAddrs[sale.payment], price - oldPrice);
        }
        else {
        ...
        tokenIdToSales[contractAddr][tokenId].offerPrices[index] = price;
        emit OfferUpdated(contractAddr, tokenId, price, msg.sender);
    }
